// Maintain Alpabetical ORDER!

exports.getCompanyBrandingDB = 'roterranet.company_branding';

exports.getActivitiesDB = 'roterranet.activities';
exports.postActivitiesDB = 'roterranet.activities';

exports.getHrTodosAnniversariesDB = 'roterranet.view_anniversary_todos';
exports.postHrTodosAnniversariesDB = 'roterranet.anniversary_todos';

exports.getCalendarCategoriesDB = 'roterranet.calendar_categories';
exports.postCalendarCategoriesDB = 'roterranet.calendar_categories';

exports.getCalendarSubCategoriesDB = 'roterranet.calendar_sub_categories';
exports.postCalendarSubCategoriesDB = 'roterranet.calendar_sub_categories';

exports.postCalendarLocationsDB = 'roterranet.calendar_locations';

exports.getContactDB = 'roterranet.view_contact';
exports.postContactDB = 'roterranet.contact';

exports.getCostControlDB = 'roterranet.view_cost_control';
exports.postCostControlDB = 'roterranet.cost_control';

exports.getCostControlItemsDB = 'roterranet.view_cost_control_items';
exports.postCostControlItemsDB = 'roterranet.cost_control_items';

exports.getCostControlInvoicingDB = 'roterranet.view_cost_control_invoicing';
exports.postCostControlInvoicingDB = 'roterranet.cost_control_invoicing';

exports.getCostControlInvoicingDetailDB = 'roterranet.view_cost_control_invoicing_items';
exports.postCostControlInvoicingDetailDB = 'roterranet.cost_control_invoicing_items';

exports.getCoursesDB = 'roterranet.courses';
exports.postCoursesDB = 'roterranet.courses';

exports.getCustomerDB = 'roterranet.view_customer';
exports.postCustomerDB = 'roterranet.customer';

exports.getCustomerTypesDB = 'roterranet.customer_types';

exports.getDepartmentsDB = 'roterranet.departments';
exports.postDepartmentsDB = 'roterranet.departments';

exports.getEmailLogDB = 'roterranet.email_log';
exports.postEmailLogDB = 'roterranet.email_log';

exports.getEquipmentDB = 'roterranet.view_equipment';
exports.postEquipmentDB = 'roterranet.equipment';

exports.getEquipmentTypesDB = 'roterranet.equipment_types';
exports.postEquipmentTypesDB = 'roterranet.equipment_types';

exports.getEquipmentScheduleDB = 'roterranet.view_equipment_schedule';
exports.postEquipmentSchedulesDB = 'roterranet.equipment_schedule';

exports.getGeneralLedgerDB = 'roterranet.general_ledger2';

exports.getGeneralLedgerDetailsDB = 'roterranet.general_ledger_details2';

exports.getHelixDiameterDB = 'roterranet.helix_diameter1';

exports.getHelixOptionsDB = 'roterranet.helix_options';

exports.getHrFilesDB = 'roterranet.view_hr_files';
exports.postHrFilesDB = 'roterranet.hr_files';

exports.getHrTodosBenefitsDB = 'roterranet.view_benefits_todos';
exports.postHrTodosBenefitsDB = 'roterranet.benefits_todos';

exports.getHrTodosRRSPDB = 'roterranet.view_rrsp_todos';
exports.postHrTodosRRSPDB = 'roterranet.rrsp_todos';

exports.getHSEFilesDB = 'roterranet.view_hse_files';
exports.postHSEFilesDB = 'roterranet.hse_files';

exports.getInAndOutDB = 'roterranet.view_calendar_3';
exports.postInAndOutDB = 'roterranet.calendar_events_main';

exports.getIndustriesDB = 'roterranet.industries';
exports.postIndustriesDB = 'roterranet.industries';

exports.getItProjectsDB = 'roterranet.view_it_projects';
exports.postItProjectsDB = 'roterranet.it_projects';

exports.getMaterialTracking = 'roterranet.view_mtrs';
exports.postMaterialTracking = 'roterranet.material_tracking';

exports.getMaterialTrackingDetails = 'roterranet.view_mtr_details'; //change db name to details
exports.postMaterialTrackingDetails = 'roterranet.material_tracking_detail'; //change db name to details

exports.getMaterialTrackingFiles = 'roterranet.material_tracking_files'; //change db name to details
exports.postMaterialTrackingFiles = 'roterranet.material_tracking_files'; //change db name to details

// ///////////////
exports.getMaterialTrackingTEST = 'roterranet.view_mtr_test';
exports.postMaterialTrackingTEST = 'roterranet.material_tracking_test';

///////////////////////

exports.getNotificationSettingsDB = 'roterranet.notification_settings';

exports.getOrganizationalChartDB = 'roterranet.organizational_chart';
exports.postOrganizationalChartDB = 'roterranet.organizational_chart';

exports.getUserNotificationsDB = 'roterranet.notifications';
exports.postUserNotificationsDB = 'roterranet.notifications';

exports.getNcrDepartmentsDB = 'roterranet.view_ncr_departments';

exports.getNcrDB = 'roterranet.view_ncr_new';
exports.postNcrDB = 'roterranet.ncr_new';

exports.getNewsDB = 'roterranet.view_news';
exports.postNewsDB = 'roterranet.news';

exports.getNotesDB = 'roterranet.view_notes';
exports.postNotesDB = 'roterranet.notes';

exports.getPersonalPropertyDB = 'roterranet.personal_property';
exports.postPersonalPropertyDB = 'roterranet.view_personal_property';

exports.getPipesDB = 'roterranet.pipe'; //change DB name to pipes
exports.postPipesDB = 'roterranet.pipe'; //change DB name to pipes

exports.getPlatesDB = 'roterranet.plate'; //change DB name to plates
exports.postPlatesDB = 'roterranet.plate'; //change DB name to plates

exports.getPlasmaRunSheetsDB = 'roterranet.view_plasma_run_sheet';
exports.postPlasmaRunSheetsDB = 'roterranet.plasma_run_sheets';

exports.getPlasmaRunSheetItemsDB = 'roterranet.view_plasma_run_sheet_items';
exports.postPlasmaRunSheetItemsDB = 'roterranet.plasma_run_sheet_items';

exports.getPlasma_run_sheet_sizesDB = 'roterranet.view_plasma_run_sheet_sizes';
// exports.postPlasma_run_sheet_helix_sizesDB = 'roterranet.plasma_run_sheet_helix_sizes';

exports.getPlasma_run_sheet_helix_sizesDB = 'roterranet.view_plasma_run_sheet_helix_sizes';
exports.postPlasma_run_sheet_helix_sizesDB = 'roterranet.plasma_run_sheet_helix_sizes';

exports.getRoterraNetVersionsDB = 'roterranet.view_roterranet_version';
exports.postRoterraNetVersionsDB = 'roterranet.roterranet_version';
// exports.postRoterraNetVersionIdDB = 'roterranet.roterranet_version_id';

exports.getRcaDB = 'roterranet.view_rca';
exports.postRcaDB = 'roterranet.rca';

exports.getRcaTeamMembersDB = 'roterranet.view_rca_team_members';
exports.postRcaTeamMembersDB = 'roterranet.rca_team_members';

exports.getRcaImprovementsDB = 'roterranet.view_rca_improvments';
exports.postRcaImprovementsDB = 'roterranet.rca_improvments';

exports.getOffboardingChecklistsDB = 'roterranet.view_offboarding_checklists';
exports.postOffboardingChecklistsDB = 'roterranet.offboarding_checklists';

exports.getOnboardingChecklistsDB = 'roterranet.view_onboarding_checklists';
exports.postOnboardingChecklistsDB = 'roterranet.onboarding_checklists';

exports.postOrderControlDB = 'roterranet.ordercontrol';
exports.getOrderControlDB = 'roterranet.view_ordercontrol';

exports.getPoEmailListDB = 'roterranet.view_po_email_list';
exports.postPoEmailListDB = 'roterranet.po_email_list';

exports.getPoDB = 'roterranet.view_po';
exports.postPoDB = 'roterranet.po';

exports.postPoIdDB = 'roterranet.po_id'; // used for keeping assigning PO Name once approved eg. PO Name => 18100

exports.getPoStatusDB = 'roterranet.po_status';
exports.postPoStatusDB = 'roterranet.po_status';

exports.getPoPermissionsDB = 'roterranet.po_permissions';
exports.postPoPermissionsDB = 'roterranet.po_permissions';

exports.getPoDetailDB = 'roterranet.view_po_detail';
exports.postPoDetailDB = 'roterranet.po_detail';

exports.getPoReceivedDB = 'roterranet.po_received';
exports.postPoReceivedDB = 'roterranet.po_received';

exports.getPoAdvancedSearchDB = 'roterranet.view_po_advanced_search';

exports.getPoEditedDB = 'roterranet.view_po_edited';
exports.postPoEditedDB = 'roterranet.po_edited';

exports.getPoDetailEditedDB = 'roterranet.po_detail_edited';
exports.postPoDetailEditedDB = 'roterranet.po_detail_edited';

exports.getPositionsDB = 'roterranet.positions';
exports.postPositionsDB = 'roterranet.positions';

exports.getProjectsEmailListDB = 'roterranet.view_project_email_list';
exports.postProjectsEmailListDB = 'roterranet.project_email_list';

exports.getProjectsDB = 'roterranet.view_projects2';
exports.postProjectsDB = 'roterranet.projects_new2';

exports.getProjectsContactsDB = 'roterranet.view_projects_contacts';
exports.postProjectsContactsDB = 'roterranet.projects_contacts';

exports.getProjectSheetItemListDB = 'roterranet.view_project_sheet_project_list';
exports.postProjectSheetItemListDB = 'roterranet.project_sheet_project_list';

exports.getProjectSheetItemListDetailsDB = 'roterranet.view_project_sheet_item_detail';
exports.postProjectSheetItemListDetailsDB = 'roterranet.project_sheet_item_detail';

exports.getProjectsGraphApiDB = 'roterranet.projects_graph_api';

exports.getQualityFilesDB = 'roterranet.view_quality_files';
exports.postQualityFilesDB = 'roterranet.quality_files';

exports.getQuotesDB = 'roterranet.view_quotes';
exports.postQuotesDB = 'roterranet.quotes5';

exports.getQuotesAnalyticsDB = 'roterranet.view_quotes_analysis';
exports.postQuotesAnalyticsDB = 'roterranet.quotes_analytics';

exports.getQuotesCustomersDB = 'roterranet.view_quotes_customers';
exports.postQuotesCustomersDB = 'roterranet.quotes_customers';

exports.getQuotesStatusDB = 'roterranet.quotes_status';
exports.postQuotesStatusDB = 'roterranet.quotes_status';

exports.getQuotesPermissionsDB = 'roterranet.quotes_permissions';
exports.postQuotesPermissionsDB = 'roterranet.quotes_permissions';

exports.getQuotesGraphApiDB = 'roterranet.quotes_graph_api';

exports.getSalesTypeDB = 'roterranet.sales_type';

exports.getSuppliersDB = 'roterranet.view_suppliers';
exports.postSuppliersDB = 'roterranet.supplier'; // change to suppliers (add the "s" at the end)

exports.getSuppliersCriteriaDB = 'roterranet.view_suppliers_criteria';
exports.postSuppliersCriteriaDB = 'roterranet.supplier_criteria'; // change to suppliers (add the "s" at the end)

exports.getSuppliersStatusDB = 'roterranet.suppliers_status';
exports.postSuppliersStatusDB = 'roterranet.suppliers_status';

exports.getSearchDB = 'roterranet.view_search';

exports.UsersImages = 'roterranet.user_images';

exports.getUsersDB = 'roterranet.view_users';
exports.postUsersDB = 'roterranet.users';

exports.getUsersSignOutDB = 'roterranet.view_user_sign_out';
exports.postUsersSignOutDB = 'roterranet.user_sign_out';

//
exports.getUsersNewUsersCheckList = 'roterranet.view_users_new_users_checklist';
exports.postUsersNewUsersCheckList = 'roterranet.users_new_users_checklist';

exports.getUsersOffBoardingCheckList = 'roterranet.view_users_offboarding_checklist';
exports.postUsersOffBoardingCheckList = 'roterranet.users_offboarding_checklist';

//
exports.getUsersEmploymentRecordDB = 'roterranet.view_users_employment_records';
exports.postUsersEmploymentRecordDB = 'roterranet.users_employment_records';

exports.getUsersPerformanceReviewDB = 'roterranet.view_user_performance_review';
exports.postUsersPerformanceReviewDB = 'roterranet.user_performance_review';

exports.getUsersPermissionsLogs = 'roterranet.view_user_permissions_logs';
exports.postUsersPermissionsLogs = 'roterranet.user_permissions_logs';

exports.getUsersPreviousUsersDB = 'roterranet.view_users_previous_users';
exports.postUsersPreviousUsersDB = 'roterranet.users_previous_users';

exports.getUsersDisciplineReviewDB = 'roterranet.view_user_discipline_review';
exports.postUsersDisciplineReviewDB = 'roterranet.user_discipline_review';

exports.getItemTrackingItemsDB = 'roterranet.users_item_tracker_items';

exports.getItemTrackingDB = 'roterranet.view_users_item_tracking';
exports.postItemTrackingDB = 'roterranet.users_item_tracking';

exports.getUsersAllowancesDB = 'roterranet.view_users_allowance';
exports.postUsersAllowanceDB = 'roterranet.users_allowance';

exports.getUsersReviewsDB = 'roterranet.view_users_reviews';
exports.postUsersReviewsDB = 'roterranet.users_reviews';

exports.getUsersBenefitsDB = 'roterranet.view_users_benefits';
exports.postUsersBenefitsDB = 'roterranet.users_benefits';

exports.getUsersOnboardingDB = 'roterranet.view_users_onboarding';
exports.postUsersOnboardingDB = 'roterranet.users_onboarding';

exports.getUsersOffboardingDB = 'roterranet.view_users_offboarding';
exports.postUsersOffboardingDB = 'roterranet.users_offboarding';

exports.getUsersCoursesDB = 'roterranet.view_users_courses';
exports.postUsersCoursesDB = 'roterranet.users_courses';

exports.getUsersPermissionsDB = 'roterranet.view_users_permissions';

exports.getUsersPermissionsLogsDB = 'roterranet.view_users_permissions_logs';
exports.postUsersPermissionsLogsDB = 'roterranet.users_permissions_logs';

exports.getUserRightsDB = 'roterranet.user_rights'; // Change to users_rights => one day

exports.getUsersHierarchyDB = 'roterranet.view_users_hierarchy';

exports.getWorkordersDB = 'roterranet.view_workorders';
exports.postWorkordersDB = 'roterranet.workorders_new'; // switch to regular workorders
exports._addworkorderFN = 'roterranet._addworkorderFN';

exports.getWorkorderFilesDB = 'roterranet.view_workorder_files';
exports.postWorkorderFilesDB = 'roterranet.workorder_files';

exports.getWorkorderIdDB = 'roterranet.workorder_id';
exports.postWorkorderIdDB = 'roterranet.workorder_id';

exports.getWorkordersStatusDB = 'roterranet.workorders_status';
exports.postWorkordersStatusDB = 'roterranet.workorders_status';

exports.getWorkordersHeatsDB = 'roterranet.view_workorder_heats_new';
exports.postWorkordersHeatsDB = 'roterranet.workorders_heats';

exports.getWorkordersItemsDB = 'roterranet.view_workorders_items';
exports.postWorkordersItemsDB = 'roterranet.workorders_items_new2';

exports.getWorkordersItemsDetailsDB = 'roterranet.view_workorders_items_details';
exports.postWorkordersItemsDetailsDB = 'roterranet.workorders_items_details';

exports.getWorkordersItemsDetailsWeldersDB = 'roterranet.view_workorders_items_details_welders';
exports.postWorkordersItemsDetailsWeldersDB = 'roterranet.workorders_items_details_welders';

exports.getWorkordersItemsDetailsInspectionsDB = 'roterranet.workorders_items_details_inspections'; //change to view eventually
exports.postWorkordersItemsDetailsInspectionsDB = 'roterranet.workorders_items_details_inspections';

exports.getWorkordersItemsDetailsQuestionsDB = 'roterranet.workorders_items_details_questions';
exports.postWorkordersItemsDetailsQuestionsDB = 'roterranet.workorders_items_details_questions';
